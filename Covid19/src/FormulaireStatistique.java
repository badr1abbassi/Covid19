...
public class FormulaireStatistique {
	...
	String tableauRegions[] = {"Fès-Meknès", "Oriental", "Guelmim-Oued Noun" ,
			"Souss-Massa", "Drâa-Tafilalet", "Marrakech-Safi", "Casablanca-Settat",
			"Béni Mellal-Khénifra", "Tanger-Tétouan-Al Hoceïma", "Rabat-Salé-Kénitra", "Dakhla-Oued Ed Dahab", "Laâyoune-Sakia El Hamra"};
	...
	public void insertStatistiques() {
		...
		for(int i=0;i<12;i++) {
			FirestoreClient.getFirestore().collection("national").document("nationalStatistique").collection("regions").document(tableauRegions[i]).collection("statistiques").add(list.get(i).getHashMap());
		}
